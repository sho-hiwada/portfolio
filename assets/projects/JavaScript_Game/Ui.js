export class UI {
  ...
  draw(context) {
    ...
    // energy
    context.font = this.fontSize * 0.8 + 'px ' + this.fontFamily;
    context.fillText('Energy: ' + this.game.energy.toFixed(0), 40, 220);
    // lives
    for (let i = 0; i < this.game.lives; i++){
    context.drawImage(this.livesImage, 50 * i + 40, 250, 50, 50);
    }
    // game over message
    if(this.game.gameOver) {
      ...
      context.font = this.fontSize * 0.7 + 'px ';
      context.fillText('[ Q ]キー もしくは 下へスワイプでリスタート', this.game.width * 0.5, this.game.height * 0.5 + 180);
